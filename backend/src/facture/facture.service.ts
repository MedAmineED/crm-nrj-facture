...
export class FactureService {
  ...
  private extractDataFromText(
    ...
    const addressStartIndex = textItems.findIndex(
      (item) => item.text.trim() === 'Votre adresse du site de consommation',
    );
    ...
    if (extractedData['adresse_site']) {
      const cpMatch = extractedData['adresse_site'].match(/\b(\d{5})\b/);
      if (cpMatch) {
    ...
    const pdlMatch = text.match(
      /N° de Point de Rel[èe]ve et de Mesure\s*[:\-]?\s*([\d]+)/,
    );
    const pceMatch = text.match(/N° de PCE\s*[:\-]?\s*([\d]+)/);
    extractedData['pdl'] =
      pdlMatch && pceMatch
        ? `PDL: ${pdlMatch[1]} / PCE: ${pceMatch[1]}`
        : pceMatch
          ? `PCE: ${pceMatch[1]}`
          : pdlMatch
            ? `PDL: ${pdlMatch[1]}`
